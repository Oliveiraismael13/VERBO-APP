...
type LibraryRow = { favorites_json: string; highlights_json: string; notes_json: string; plans_json: string; shared_json?: string; found_scrolls_json?: string; last_reading_json?: string };
const empty = { favorites: [] as string[], highlights: {} as Record<string, string>, notes: {} as Record<string, string>, plans: [] as string[], shared: [] as string[], foundScrolls: [] as string[], lastReading: null as LastReading | null };
...
export async function GET() {
  ...
  await ensureSchema();
  const row = await env.DB.prepare("SELECT favorites_json, highlights_json, notes_json, plans_json, shared_json, found_scrolls_json, last_reading_json FROM user_library WHERE user_id = ?").bind(user.id).first<LibraryRow>();
  return withCors(Response.json(row ? { favorites: parse(row.favorites_json, []), highlights: parse(row.highlights_json, {}), notes: parse(row.notes_json, {}), plans: parse(row.plans_json, []), shared: parse(row.shared_json, []), foundScrolls: parse(row.found_scrolls_json, []), lastReading: parseLastReading(parse(row.last_reading_json, null)) } : empty));
}
...
export async function PATCH(request: Request) {
  ...
  await ensureSchema();
  await env.DB.prepare("INSERT INTO user_library (user_id, favorites_json, highlights_json, notes_json, plans_json, shared_json, found_scrolls_json, last_reading_json, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET favorites_json = excluded.favorites_json, highlights_json = excluded.highlights_json, notes_json = excluded.notes_json, plans_json = excluded.plans_json, shared_json = excluded.shared_json, found_scrolls_json = excluded.found_scrolls_json, last_reading_json = excluded.last_reading_json, updated_at = excluded.updated_at").bind(user.id, JSON.stringify(favorites), JSON.stringify(highlights), JSON.stringify(notes), JSON.stringify(plans), JSON.stringify(shared), JSON.stringify(foundScrolls), JSON.stringify(lastReading), Date.now()).run();
  return withCors(Response.json({ favorites, highlights, notes, plans, shared, foundScrolls, lastReading }));
}
