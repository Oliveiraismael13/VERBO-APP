...
type LibraryRow = { favorites_json: string; highlights_json: string; notes_json: string; note_dates_json?: string; plans_json: string; shared_json?: string; found_scrolls_json?: string; last_reading_json?: string };
const empty = { favorites: [] as string[], highlights: {} as Record<string, string>, notes: {} as Record<string, string>, noteDates: {} as Record<string, number>, plans: [] as string[], shared: [] as string[], foundScrolls: [] as string[], lastReading: null as LastReading | null };
...
export async function GET() {
  ...
  await ensureSchema();
  const row = await env.DB.prepare("SELECT favorites_json, highlights_json, notes_json, note_dates_json, plans_json, shared_json, found_scrolls_json, last_reading_json FROM user_library WHERE user_id = ?").bind(user.id).first<LibraryRow>();
  return withCors(Response.json(row ? { favorites: parse(row.favorites_json, []), highlights: parse(row.highlights_json, {}), notes: parse(row.notes_json, {}), noteDates: parse(row.note_dates_json, {}), plans: parse(row.plans_json, []), shared: parse(row.shared_json, []), foundScrolls: parse(row.found_scrolls_json, []), lastReading: parseLastReading(parse(row.last_reading_json, null)) } : empty));
}
