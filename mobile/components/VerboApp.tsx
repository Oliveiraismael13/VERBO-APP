...
function PixelDisciple({ turning = false }: { turning?: boolean }) {
  // Sprites fornecidos para o personagem da campanha; mantém os pixels nítidos em qualquer tela.
  if (turning) {
    const directions = ["south", "south-west", "west", "north-west", "north", "north-east", "east", "south-east"];
    return <span className="pixel-disciple-turn">{directions.map((direction, index) => <img key={direction} src={`/characters/homem-50lvl-idle-${direction}.png`} alt="" aria-hidden="true" style={{ animationDelay: `${-index * 0.4}s` }} />)}</span>;
  }
  // eslint-disable-next-line @next/next/no-img-element
