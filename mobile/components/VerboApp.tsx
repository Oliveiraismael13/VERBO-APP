...
function JourneyPage({ manifest, progress, onContinue, onOpenBible }: { manifest: BibleManifest | null; progress: PlayerProgress; onContinue: () => void; onOpenBible: () => void }) {
  ...
  return <section className="journey-page page-in">
    ...
    <div className="daily-quests">
      ...
      <article><i>🔥</i><div><b>Mantenha a Chama acesa</b><span>{progress.streak} {progress.streak === 1 ? "dia consecutivo" : "dias consecutivos"} no Verbo</span></div><strong>🔥 sequência</strong></article>
...
function MissionStoryPanel({ context, progress }: { context: ReturnType<typeof missionForChapter>; progress: PlayerProgress }) {
  ...
  const progressInAct = actProgress(context.act, progress.completed);
  const progressInStage = stageProgress(context.mission, progress.completed);
  return <aside className="mission-story-panel"><div><p>MISSÃO {index} DE {context.act.missions.length} · ATO {context.act.number}</p><h2>{context.mission.title}</h2><span>{narrative.introduction}</span></div><div className="mission-story-progress"><div className="mission-stage-progress"><small>CAPÍTULOS PARA CONCLUSÃO</small><b>{progressInStage.done}/{progressInStage.total}</b><i><em style={{ width: `${progressInStage.percent}%` }} /></i></div><div><small>ATO</small><b>{progressInAct.done}/{progressInAct.total}</b><i><em style={{ width: `${progressInAct.percent}%` }} /></i></div></div><blockquote><small>CONTEXTO HISTÓRICO</small>{narrative.historicalContext}</blockquote></aside>;
}
