...
function JourneyPage({ manifest, progress, activeSecondaryMission, activeSecondaryStatus, replayingSecondaryMission, onContinue, onContinueSecondary, onOpenBible, onOpenCoop, onRestoreStreak, onCoopChange }: { manifest: BibleManifest | null; progress: PlayerProgress; activeSecondaryMission: SecondaryMission | null; activeSecondaryStatus: SecondaryMissionStatus | null; replayingSecondaryMission: boolean; onContinue: () => void; onContinueSecondary: (mission: SecondaryMission) => void; onOpenBible: () => void; onOpenCoop: (reference: CoopReference) => void; onRestoreStreak: () => void; onCoopChange: (coop: CoopMissionState) => void }) {
  ...
  const currentAct = campaignActs.find((act) => !isActComplete(act, progress.completed));
  const currentActProgress = currentAct ? actProgress(currentAct, progress.completed) : null;
  ...
  return <section className="journey-page page-in">
    ...
    <nav className="journey-shortcuts" aria-label="Explorar a jornada">
      <a href="#daily-focus">Missões diárias <span aria-hidden="true">↓</span></a>
      <a href="#journey-map">Ver minha trilha <span aria-hidden="true">↓</span></a>
    </nav>
    <article className={`active-quest ${activeSecondaryMission ? "secondary-active-quest" : ""}`}>
      ...
      {currentAct && currentActProgress && !activeSecondaryMission && <section className="quest-act-progress" aria-label={`Progresso no ato ${currentAct.number}`}>
        <div><span>Ato {currentAct.number} · {currentAct.title}</span><b>{currentActProgress.done}/{currentActProgress.total}</b></div>
        <progress value={currentActProgress.done} max={currentActProgress.total} aria-label="Capítulos concluídos neste ato" />
      </section>}
    ...
    <section className="journey-daily" id="daily-focus" aria-labelledby="daily-focus-title">
    <div className="daily-title"><div><p>MISSÕES DIÁRIAS</p><h2 id="daily-focus-title">Fortaleça sua constância</h2></div><span>◴ 24h</span></div>
    ...
    <section className="journey-trail" id="journey-map" aria-labelledby="journey-map-title">
    <div className="quest-heading"><div><p>TRILHA PRINCIPAL</p><h2 id="journey-map-title">A Grande História</h2></div><span>{completedCount}/1.189</span></div>
    <div className="quest-map">
      <i className="map-line" />
      {milestones.map((item, index) => <button key={item.name} className={`map-node ${item.state} ${index % 2 ? "right" : "left"}`} onClick={item.state === "locked" ? undefined : onOpenBible} disabled={item.state === "locked"} aria-current={item.state === "current" ? "step" : undefined}>
...
function SocialPostDetail({ text }: { text: string }) {
  const [expanded, setExpanded] = useState(false);
  const canExpand = text.length > 160 || text.includes("\n");
  return <div className="social-post-detail">
    <p>{canExpand && !expanded ? `${text.slice(0, 160).trimEnd()}…` : text}</p>
    {canExpand && <button type="button" aria-expanded={expanded} onClick={() => setExpanded(!expanded)}>{expanded ? "Ver menos" : "Ler publicação completa"}</button>}
  </div>;
}
...
function SocialPage({ notify, dark, setDark, progress, manifest, onOpenFavorite, onProfilePhotoChange, onDisplayNameChange }: { notify: (message: string) => void; dark: boolean; setDark: (value: boolean) => void; progress: PlayerProgress; manifest: BibleManifest | null; onOpenFavorite: (reference: string) => void; onProfilePhotoChange: (file: File) => void; onDisplayNameChange: (displayName: string) => Promise<boolean> }) {
  ...
  const loadSocial = useCallback(async () => {
    ...
    setLoadError([friendsResponse, privacyResponse, feedResponse, notificationsResponse, blocksResponse, notesResponse].some((response) => !response.ok));
  }, []);

  useEffect(() => {
    const task = window.setTimeout(() => void loadSocial().catch(() => setLoadError(true)).finally(() => setLoading(false)), 0);
    return () => window.clearTimeout(task);
  }, [loadSocial]);

  useEffect(() => {
    const refreshFeed = () => {
      if (document.visibilityState === "visible") void loadSocial().catch(() => setLoadError(true));
    };
  ...
  return <section className="generic-page social-page social-home page-in">
    ...
    {loadError && <div className="social-load-error" role="status"><p>Não foi possível atualizar todas as conexões. Confira sua conexão e tente novamente.</p><button disabled={loading} onClick={() => { setLoading(true); void loadSocial().catch(() => setLoadError(true)).finally(() => setLoading(false)); }}>Tentar novamente</button></div>}
    {loading ? <div className="social-loading" role="status"><span className="social-loading-dot" aria-hidden="true" />Carregando suas conexões…</div> : <>
      {socialView === "feed" && <button className="social-compose-prompt" onClick={() => { if (!privacy?.showActivities) { notify("Ative o compartilhamento nas preferências para publicar para seus amigos"); openAccount(); return; } setComposerOpen(true); }}><span aria-hidden="true">✎</span><span><b>O que a Palavra te mostrou hoje?</b><small>Compartilhe uma reflexão ou testemunho</small></span><span aria-hidden="true">＋</span></button>}
      {socialView === "feed" && <section className="social-section social-feed"><div className="social-section-title social-feed-title"><div><p className="eyebrow">FEED DE AMIGOS</p><h2>Na sua comunidade</h2></div><span>{activities.length}</span></div><div className="social-feed-tabs" role="tablist" aria-label="Ordenar atividades"><button role="tab" aria-selected={feedOrder === "recent"} className={feedOrder === "recent" ? "active" : ""} onClick={() => setFeedOrder("recent")}>Recentes</button><button role="tab" aria-selected={feedOrder === "celebrated"} className={feedOrder === "celebrated" ? "active" : ""} onClick={() => setFeedOrder("celebrated")}>Celebradas</button></div>{orderedActivities.length ? <div className="social-feed-list">{orderedActivities.map((activity) => <article id={`social-activity-${activity.id}`} key={activity.id} className={`social-feed-card activity-${activity.kind}${activity.category ? ` activity-${activity.category}` : ""}${focusedActivityId === activity.id ? " focused" : ""}`}><button className="social-activity-head" onClick={() => void openProfile(activity.actor.publicHandle, "feed")}><SocialAvatar contact={activity.actor} small /><span><b>{activity.actor.displayName}</b><small>{new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "short" }).format(new Date(activity.createdAt))} · Ver jornada</small></span><em aria-hidden="true">›</em></button><div className="social-activity-copy"><span className="social-activity-glyph" aria-hidden="true">{activityGlyph(activity)}</span><div><i>{activityLabel(activity)}</i>{activity.category === "reflection_shared" || activity.category === "testimony_shared" ? <SocialPostDetail text={activity.title} /> : <p>{activity.title}</p>}{activity.detail && <SocialPostDetail text={activity.detail} />}{(activity.xp || activity.level) && <span className="social-activity-reward">{activity.xp ? `+${activity.xp} XP` : ""}{activity.xp && activity.level ? " · " : ""}{activity.level ? `Nível ${activity.level}` : ""}</span>}{activity.reference && <button className="social-activity-reference" onClick={() => onOpenFavorite(activity.reference!)}>▥ {activity.reference.includes(":") ? socialReference(activity.reference, manifest) : activity.reference} ›</button>}</div></div><div className="social-reactions"><button className={activity.reactions.viewer === "amen" ? "active" : ""} aria-label={`Amém, ${activity.reactions.amen} reações`} aria-pressed={activity.reactions.viewer === "amen"} disabled={working} onClick={() => void reactToActivity(activity, "amen")}>🙏 <span>{activity.reactions.amen || "Amém"}</span></button><button className={activity.reactions.viewer === "celebrate" ? "active" : ""} aria-label={`Celebrar, ${activity.reactions.celebrate} reações`} aria-pressed={activity.reactions.viewer === "celebrate"} disabled={working} onClick={() => void reactToActivity(activity, "celebrate")}>✦ <span>{activity.reactions.celebrate || "Celebrar"}</span></button><button className={commentActivityId === activity.id ? "active social-comment-toggle" : "social-comment-toggle"} aria-expanded={commentActivityId === activity.id} aria-controls={`social-comments-${activity.id}`} onClick={() => setCommentActivityId((current) => current === activity.id ? null : activity.id)}>◌ <span>{activity.comments.total ? `${activity.comments.total} comentário${activity.comments.total === 1 ? "" : "s"}` : "Comentar"}</span></button></div>{(activity.comments.items.length > 0 || commentActivityId === activity.id) && <div className="social-comments" id={`social-comments-${activity.id}`}>{activity.comments.items.map((comment) => <div className="social-comment" key={comment.id}><SocialAvatar contact={comment.author} small /><p><b>{comment.author.displayName}</b>{comment.text}</p></div>)}{commentActivityId === activity.id && <form className="social-comment-form" onSubmit={(event) => { event.preventDefault(); void publishComment(activity); }}><input value={commentDrafts[activity.id] || ""} maxLength={400} onChange={(event) => setCommentDrafts((current) => ({ ...current, [activity.id]: event.target.value }))} placeholder="Escreva algo que edifique…" aria-label="Seu comentário" autoFocus /><button disabled={working || !(commentDrafts[activity.id] || "").trim()}>Enviar</button></form>}</div>}</article>)}</div> : <div className="social-feed-empty"><span>✦</span><h3>Sua comunidade começa aqui</h3><p>Compartilhe uma reflexão ou encontre amigos para acompanhar suas jornadas na Palavra.</p><button className="social-empty-action" onClick={() => setSocialView("discover")}>Encontrar pessoas →</button></div>}</section>}
